...
(K-beta (A : U) (B : U) (x : A) (y : B)) :
  (eq A A (app B A (app A (Arr B A) (K A B) x) y) x)
(flip-K (A : U) (B : U)) : (Arr A (Arr B B))
(flip-K-beta (A : U) (B : U) (x : A) (y : B)) :
  (eq B B (app B B (app A (Arr B B) (flip-K A B) x) y) y)
(I (A : U)) : (Arr A A)
(I-beta (A : U) (x : A)) :
  (eq A A (app A A (I A) x) x)
-- Congruences
(app-cong (A : U) (B : U) (f : (Arr A B)) (g : (Arr A B)) (x : A) (e : (eq (Arr A B) (Arr A B) f g))) : (eq B B (app A B f x) (app A B g x))
-- ForallArr0: forall M:U. M — empty type
ForallArr0 : U
(inst0 (M : U) (f : ForallArr0)) : M
-- ForallArr1: forall M:U. Arr M M — one inhabitant: polyId
ForallArr1 : U
(inst1 (M : U) (f : ForallArr1)) : (Arr M M)
polyId : ForallArr1
(polyId-beta (M : U)) :
  (eq (Arr M M) (Arr M M) (inst1 M polyId) (I M))
-- ForallArr2: forall M:U. Arr M (Arr M M) — two inhabitants
ForallArr2 : U
(inst2 (M : U) (f : ForallArr2)) : (Arr M (Arr M M))
polyK : ForallArr2
(polyK-beta (M : U)) :
  (eq (Arr M (Arr M M)) (Arr M (Arr M M)) (inst2 M polyK) (K M M))
polyFlipK : ForallArr2
(polyFlipK-beta (M : U)) :
  (eq (Arr M (Arr M M)) (Arr M (Arr M M)) (inst2 M polyFlipK) (flip-K M M))
meta
