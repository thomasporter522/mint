-- Tests chained reductions of different shapes mixed with cong.
...
-- A pile of related reductions for testing chains.
postulate
A1 : U
A2 : U
A3 : U
...
meta
    ...
    schema s4 = fun ctx => fun args => (Ok [val-of-A4])

-- 4-step reduction chain: A1 → A2 → A3 → A4. Witness has type A4.
construct by s4
t-a1 : A1

-- Witnesses for sto-typed decls — need a sto-typed value.
postulate
sto-witness : sto A4 A4
meta
    schema s-sto = fun ctx => fun args => (Ok [sto-witness])

-- nested in sto on one arg
construct by s-sto
t-sto-A1 : sto A1 A4

-- Both args reduce.
construct by s-sto
t-sto-A1-A1 : sto A1 A1
