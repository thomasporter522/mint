-- Tests of deeper reductions and combinations: multi-step reduce,
-- cong + recursive convert, nested cases.
...
-- Chain of reductions: w1 -> w2 -> w3 -> base
postulate
base : U
w1 : U
w2 : U
...
meta
    ...
    schema base-s = fun ctx => fun args => (Ok [test-base])
    -- Schema that emits a sto-typed witness using sto-cong over refls.
    schema sto-s = fun ctx => fun args =>
        (Ok [(sto-cong refl refl)])

-- Test: 3-step reduction. Witness has type base, decl expects w1.
construct by base-s
t-chain : w1

-- Test: sto reduction at both args. Witness is base-typed, the kernel
-- would already need a sto-typed witness; switch schema.
postulate
sto-base-base : sto base base
meta
    schema sto-base-s = fun ctx => fun args => (Ok [sto-base-base])
construct by sto-base-s
t-sto-redex : sto w1 w2

-- Test: 2-level nested reduction.
postulate
sto-base-sto : sto base (sto base base)
meta
    schema sto-deep-s = fun ctx => fun args => (Ok [sto-base-sto])
construct by sto-deep-s
t-deep : sto base (sto w1 w3)
