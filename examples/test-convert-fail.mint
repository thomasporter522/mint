-- Tests that conversion correctly FAILS in cases where no proof exists.
-- The kernel should report Inconsistency, not silently succeed.
...
-- No reduction between Q and P. The coerce SHOULD fail.
...
meta
    ...
    schema p-s = fun ctx => fun args => (Ok [test-p])

-- This SHOULD fail: P and Q are unrelated postulates. Coerce can't bridge.
construct by p-s
bad : Q
