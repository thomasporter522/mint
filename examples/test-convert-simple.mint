-- Smallest test that actually exercises the coerce. A schema produces
-- a witness whose type differs from the decl's expected type but only
-- by a #reduction step — the coerce should auto-bridge.
...
meta
    ...
    apply-cong : (Term -> ((List Term) -> (Result Term))) = fun head => fun proofs =>
        (Error "no cong rule for this head in this test")
    ...
    -- Schema for the test: the construct block has one decl whose
    -- TYPE is `redex TestP`. The schema provides `test-witness`
    -- (which has type TestP). The coerce should produce a cast.
    schema test-schema = fun ctx => fun args => (Ok [test-witness])

construct by test-schema
test : redex TestP
