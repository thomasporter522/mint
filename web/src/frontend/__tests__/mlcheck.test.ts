...
describe('ML type checker: basic lambda', () => {
  it('accepts a lambda returning Ok []', () => {
    ok('fun outer => fun s => (Ok [])');
  });

  it('accepts a lambda returning Error', () => {
    ok('fun outer => fun s => (Error "bad")');
  });
...
describe('ML type checker: Ok/Error', () => {
  it('Ok wraps a list of terms', () => {
    ok('fun outer => fun s => (Ok [x, y, z])');
  });

  it('Ok with single element list', () => {
    ok('fun outer => fun s => (Ok [x])');
  });

  it('Error requires a string argument', () => {
    ok('fun outer => fun s => (Error "something went wrong")');
  });

  it('Error rejects non-string argument', () => {
    fails('fun outer => fun s => (Error x)', 'Expected String');
  });

  it('Ok rejects non-list argument', () => {
    fails('fun outer => fun s => (Ok x)', 'Expected List');
  });
});
...
describe('ML type checker: pattern matching', () => {
  it('accepts match with wildcard', () => {
    ok('fun outer => fun s => match s with | _ => (Ok []) end');
  });

  it('accepts match with variable pattern', () => {
    ok('fun outer => fun s => match s with | x => (Ok []) end');
  });

  it('accepts match with list patterns', () => {
    ok('fun outer => fun s => match s with | [] => (Error "empty") | _ => (Ok []) end');
  });
  ...
  it('accepts match with multiple branches', () => {
    ok('fun outer => fun s => match s with | [] => (Error "empty") | [_] => (Ok []) | _ => (Error "too many") end');
  });

  it('rejects branch returning wrong type', () => {
    fails('fun outer => fun s => match s with | _ => "wrong" end');
  });

  it('rejects mismatched branch types', () => {
    // First branch returns Result, second returns string
    fails('fun outer => fun s => match s with | [] => (Ok []) | _ => "wrong" end');
  });
});
...
describe('ML type checker: if/then/else', () => {
  it('accepts if/then/else returning Result', () => {
    ok('fun outer => fun s => (if s == s then (Ok []) else (Error "bad") end)');
  });

  it('rejects if with mismatched branch types', () => {
    fails('fun outer => fun s => (if s == s then (Ok []) else "wrong" end)');
  });
});
...
describe('ML type checker: nested structures', () => {
  it('match inside if', () => {
    ok('fun outer => fun s => (if s == s then match s with | _ => (Ok []) end else (Error "bad") end)');
  });

  it('if inside match branch', () => {
    ok('fun outer => fun s => match s with | _ => if s == s then (Ok []) else (Error "bad") end end');
  });

  it('nested match', () => {
    ok('fun outer => fun s => match s with | [first] => match first with | _ => (Ok []) end | _ => (Error "bad") end');
  });
});
...
describe('ML type checker: full schema example', () => {
  ...
  it('rejects schema that returns string', () => {
    fails('fun outer => fun s => match s with | _ => "not a result" end');
  });

  it('rejects schema that forgets Error wrapper', () => {
    fails('fun outer => fun s => match s with | _ => "message" end');
  });
});
...
describe('ML type checker: x binding', () => {
  ...
  it('bare ? is a wildcard, not a named binding', () => {
    ok('fun outer => fun s => match s with | [?] => (Ok []) | _ => (Error "bad") end');
  });
...
describe('ML type checker: signature type', () => {
  ...
  it('old 2-tuple pattern fails on signature', () => {
    ...
    fails('fun outer => fun s => match s with | [(params, ret)] => (Ok [ret]) | _ => (Error "bad") end', 'Tuple pattern');
  });
...
describe('ML type checker: Term is not a wildcard', () => {
  ...
  it('Term where Bool expected is rejected', () => {
    fails('fun outer => fun s => (if x then (Ok []) else (Error "bad") end)');
  });
  ...
  it('let binding preserves inferred type', () => {
    ...
    fails('fun outer => fun s => let x = [a, b] in (Ok [x])', 'Expected Term');
  });
  ...
  it('let binding of string is typed as string', () => {
    fails('fun outer => fun s => let x = "hi" in (Ok [x])', 'Expected Term');
  });

  it('match branch types must agree', () => {
    // First branch returns Result (List Term), second must too
    fails('fun outer => fun s => match s with | [] => (Ok []) | _ => [] end');
  });

  it('if branches must have same type', () => {
    fails('fun outer => fun s => (if true then (Ok []) else [] end)');
  });
});
...
describe('ML type checker: let type annotations', () => {
  ...
  it('annotated let with Term type', () => {
    ok('fun outer => fun s => let x : Term = a in (Ok [x])');
  });

  it('annotated let with List Term type', () => {
    ok('fun outer => fun s => let xs : (List Term) = [a, b] in (Ok xs)');
  });

  it('annotated let with String type', () => {
    ok('fun outer => fun s => let msg : String = "hi" in (Error msg)');
  });

  it('annotated let with arrow type', () => {
    ok('fun outer => fun s => let f : (Term -> Term) = fun x => x in (Ok [(f a)])');
  });

  it('annotated let with pair type', () => {
    ok('fun outer => fun s => let p : (Term, Term) = (a, b) in (Ok [fst p])');
  });
  ...
  it('annotated let with List Term -> List Term', () => {
    ok('fun outer => fun s => let rev : ((List Term) -> (List Term)) = fun xs => (foldl (fun acc => fun x => x :: acc) [] xs) in (Ok (rev [a, b]))');
  });
  ...
  it('annotation gives param types to lambda', () => {
    ...
    ok('fun outer => fun s => let rev : ((List Term) -> (List Term)) = fun xs => (foldl (fun acc => fun x => x :: acc) [] xs) in (Ok (rev [a]))');
  });

  it('annotated let with Result type', () => {
    ok('fun outer => fun s => let r : (Result (List Term)) = (Ok [a]) in r');
  });
  ...
  it('annotation propagates through function application', () => {
    ...
    ok('fun outer => fun s => let rev : ((List Term) -> (List Term)) = fun xs => (foldl (fun acc => fun x => x :: acc) [] xs) in match s with | _ => (Ok (rev [a, b])) end');
  });

  /* --- Annotations that should fail --- */

  it('rejects body that does not match annotation', () => {
    fails('fun outer => fun s => let x : String = [a, b] in (Error x)', 'Expected String');
  });

  it('rejects using annotated binding at wrong type', () => {
    // x : String, but Ok wants List Term
    fails('fun outer => fun s => let x : String = "hi" in (Ok [x])', 'Expected Term');
  });

  it('rejects arrow annotation on non-function', () => {
    fails('fun outer => fun s => let f : (Term -> Term) = a in (Ok [(f a)])', 'Expected Term -> Term');
  });

  it('rejects List annotation on non-list body', () => {
    fails('fun outer => fun s => let xs : (List Term) = a in (Ok xs)', 'Expected List');
  });

  it('rejects pair annotation on non-pair body', () => {
    fails('fun outer => fun s => let p : (Term, Term) = a in (Ok [fst p])', 'Expected (Term, Term)');
  });
  ...
  it('annotation mismatch is caught even when body type-checks in isolation', () => {
    ...
    fails('fun outer => fun s => let xs : String = [a, b] in (Error xs)', 'Expected String');
  });
});
